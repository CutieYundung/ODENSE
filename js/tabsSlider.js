        ...
        class s {
          ...
          _init() {
            (this.tabsBarWrap = this.tabs.querySelector(".tabs__bar-wrap")),
              ...
              (this.controls = Array.prototype.slice.call(
                this.bar.querySelectorAll(".tabs__controls")
              )),
              (this.sections = Array.prototype.slice.call(
                this.content.querySelectorAll(".tabs__section")
              )),
              (this.offset = 0),
              (this.currentId = this.settings.slide),
              (this.slidesLen = this.sections.length),
              (this.tabsHasOverflow = !1),
          ...
          _addEvents() {
            (this._handlerClick = this._selectTab.bind(this)),
              ...
              this.content.addEventListener("focus", this._handlerTabFocus, !0),
          ...
          _moveSliderLine() {
            const { offsetWidth: t, offsetLeft: e } =
          ...
          _checkTabsOverflow() {
            ...
            const t =
                this.tabsHasOverflow &&
                this.bar.scrollWidth >
                  this.bar.scrollLeft + this.bar.offsetWidth,
              e = this.bar.scrollLeft > 0;
            this.tabsBarWrap.classList.toggle("has-right-overflow", t),
              this.tabsBarWrap.classList.toggle("has-left-overflow", e);
          }
          _observeTabInViewport() {
            if (!this.tabsHasOverflow) return;
            const t = this.controls[this.currentId],
              { offsetWidth: e, offsetLeft: s } = t;
            let i = this.bar.scrollLeft;
            s <= this.bar.scrollLeft
              ? (i = s)
              : s + e >= this.bar.scrollLeft + this.bar.offsetWidth &&
                (i = this.bar.scrollLeft + this.bar.offsetWidth - e),
              this._scrollLeft(this.bar, { to: i }, () => {
                this._checkTabsOverflow();
              });
          }
          _dimmensions() {
          ...
          _moveSlide(t, e) {
            if ((void 0 === e && (e = !0), this.settings.animate)) {
              let t = e ? this.settings.duration : 0,
                s = [
                  `${this.transformProperty} ${t}ms ${this.settings.easing}`,
                ];
              this.settings.heightAnimate &&
                s.push(`height ${t}ms ${this.settings.easing}`),
                (this.content.style[this.transitionProperty] = s.join(","));
            }
            this.content.style[
          ...
          _start(t) {
            ...
            let e;
            t.targetTouches
              ? ((this.target = t.targetTouches[0].target),
                (e = t.targetTouches[0]))
              : ((e = t), t.preventDefault()),
              (this.delta = 0),
              (this.dragX = e.pageX || e.clientX),
              (this.dragY = e.pageY || e.clientY),
              (this.dragFlag = !0),
          ...
          _move(t) {
            ...
            const s = e.pageX || e.clientX,
              i = e.pageY || e.clientY;
            if (
              (this.isMoving ||
                (this.isMoving =
                  Math.abs(this.dragX - s) >= Math.abs(this.dragY - i)),
              this.isMoving)
            ) {
              if (
                (t.preventDefault(),
                (this.delta = (this.dragX - s) / 2),
                !this.settings.animate)
              )
                return;
          ...
          _end() {
            ...
            if (
              ((this.isMoving = !1),
              this.content.classList.remove("has-grab"),
              Math.abs(this.delta) < 20 || t > this.slidesLen - 1 || t < 0)
            )
              return (this.dragFlag = !1), void this._moveSlide(this.offset);
          ...
          show(t) {
            (t = Math.abs(t)) >= this.slidesLen && (t = this.slidesLen - 1),
              this.controls[this.currentId].classList.remove("is-active");
